...
from AV_Losses import gamma_training, KL_metric, recon_metric, vae_training
from AV_Layers import VAE_sampling, cond_z
...
def decoder_setup(flat_units, filters, kernels, params, dec_in=None):
    ...
    FC3 = Dense(flat_units[0], name='FC3',
                   kernel_regularizer=regularizers.l2(params['l2_regularizer']),
                   bias_regularizer=regularizers.l2(params['l2_regularizer']))(dec_in) # (?, 4096)
...
if __name__ == "__main__":
    ...
    config.read('VAE-config.txt') 
    
    Model_Name = 'VAE_'+str(time.time())+'_'
    train_dir = config['directories']['train_dir']
    valid_dir = config['directories']['valid_dir']
    model_dir = config['directories']['model_dir']+'VAE/'
    filters = [64,64,16]
    kernels = [(3,3), (5,5), (5,5)]
    flat_units = [4096]
    ...
    params['annealing_factor'] = K.variable(0., dtype='float32')
    ...
    outputs = embedding_setup(params, enc_out)
    dec_in, dec_out = decoder_setup(flat_units, filters, kernels,
                                    params, dec_in=outputs['z_out'])
    with tf.device('/cpu:0'):
        enc_vae = Model(inputs=enc_in, outputs=outputs['z_mean'], name='VAE_Encoder')
        
        VAE = Model(inputs=enc_in, outputs=dec_out, name='VAE')
        ...
        tb_logs = TensorBoard(log_dir=model_dir+'/tb_logs/'+Model_Name,
                              ...
                              write_graph=False)
        checkpoints = MyModelCheckPoint(VAE, filepath=model_dir+Model_Name+'{epoch:02d}-{loss:.2f}.h5',
    ...
    vae_loss = vae_training(outputs, params)
    kl_loss = KL_metric(outputs)
    recon_loss = recon_metric(params)
    
    VAE_gpu = multi_gpu_model(VAE, gpus=2)
    
    VAE_gpu.compile(optimizers.Adam(lr=params['lr']),
                loss = vae_loss,
                metrics = [vae_loss,recon_loss,kl_loss])
    ...
    warm_up_hist = VAE_gpu.fit_generator(train_generator, epochs=params['warm_up_steps'],
    ...
    VAE.save_weights(model_dir+Model_Name+'warm_up_weights.h5')
    np.save(model_dir+Model_Name+'warm_up_losses.npy', warm_up_hist.history)
    
    x_pred = VAE_gpu.predict(x_test[0:100])
    ...
    VAE_gpu.compile(optimizers.Adam(lr=params['lr'], clipvalue=1),
                loss = vae_loss,
                metrics = [vae_loss,kl_loss,recon_loss])
    ...
    annealing_hist = VAE_gpu.fit_generator(train_generator, epochs=total_epochs,
    ...
    VAE.save_weights(model_dir+Model_Name+'final_weights.h5')
    np.save(model_dir+Model_Name+'annealing_losses.npy', annealing_hist.history)
    
    x_pred = VAE.predict(x_test[0:100])
    f, ax = plt.subplots(10,10)
    [[ax.ravel()[j].imshow(x_pred[j,:,:,0], vmin=0, vmax=1, cmap='binary') for j in range(100)]]
    plt.show()
    plt.close()
